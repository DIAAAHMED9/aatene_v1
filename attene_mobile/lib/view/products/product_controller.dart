...
class ProductController extends GetxController with SingleGetTickerProviderMixin {
  ...
  void _createTabController() {
    ...
    print('✅ [PRODUCT CONTROLLER] Created TabController with ${tabs.length} tabs, initialIndex: $initialIndex');
  }
  ...
  void _handleTabChange() {
    if (_isDisposed.value || !_isTabControllerReady.value || _tabController == null) return;
  ...
  Future<void> _loadCachedProducts() async {
    try {
      ...
      if (cachedProducts.isNotEmpty) {
        print('📂 [PRODUCT CONTROLLER] Found ${cachedProducts.length} cached products');
  ...
  Future<void> _loadProducts({bool forceRefresh = false}) async {
    ...
    if (!forceRefresh && _lastDataFetchTime != null) {
      ...
      if (difference.inSeconds < 30) {
        print('⏱️ [PRODUCT CONTROLLER] Using cached data, last fetch was ${difference.inSeconds} seconds ago');
        return;
      }
    }
  ...
  Future<void> _performLoadProducts() async {
    ...
    try {
      ...
      if (response != null && response['status'] == true) {
        ...
        final newProducts = data.map((product) => Product.fromJson(product)).toList();
  ...
  void _updateTabsWithSections({bool forceUpdate = false}) {
    if (_isDisposed.value || !_isInitialized.value || _isUpdatingTabs.value || !_isTabControllerReady.value) return;
  ...
  Future<void> _loadTabData(int tabIndex) async {
    try {
      if (tabIndex < tabs.length) {
        print('📋 [PRODUCT CONTROLLER] Loading tab data: ${tabs[tabIndex].label}');
      }
    } catch (e) {
  ...
  Map<String, List<Product>> get productsBySection => Map.from(_productsBySection);
  ...
  bool get isTabControllerReady => _isTabControllerReady.value && _tabController != null;
